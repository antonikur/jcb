...
public class TestFiturRejectSurveyor {
	...
	@Test(priority = 1)
	public void cek_fitur_filter_reject() {
		...
		home.clickAndGotoMenuReject();
		JCBRejectPage reject = PageFactory.initElements(driver, JCBRejectPage.class);

		reject.clickBtnFilter();
		tools.stopForMoment(2000);
		reject.selectDropdownListEntriesByValue("76");
		tools.stopForMoment(2000);
		reject.clickBtnFilter();
		String selected = reject.getSelectedOption();

		tools.stopForMoment(2000);
		String textArea = reject.clickBtnVisitAndGoToVisitPage().getTextArea();
	...
	@Test(priority = 2)
	public void cek_fitur_search_reject() {
		...
		JCBRejectPage reject = PageFactory.initElements(driver, JCBRejectPage.class);
		tools.stopForMoment(2000);
		reject.clickSearchField(keyword);
		tools.stopForMoment(1000);
		reject.clickBtnSearch();
		tools.stopForMoment(1000);
		boolean isCorrect = false;

		List<WebElement> columnName = reject.getListTableColumnName();
